...
while done == False:
	...
	theta = n * 2*PI/ntotal
	#R = R1*(1+math.cos(theta/2)+1)
	#r = r1*(1+math.cos(theta/6)+1)
	R += 0.0003*sign
	r = R
	if (R>100.0) or (R<0.00001):
#		r=0.0
#		R=0.0
		sign*=-1
	...
	animation_speed+=1
	if (animation_speed%2==0):    ## the modulo based gave the impression of forward movement
		delta += 0.0005   # by adjusting this with the animation modulo above it is possible to adjust the speed forward without the effect of "jaggedness" in the display
        cx=int(120*(math.sin((theta-delta)/5)+1)+10)
	cy=int(120*(math.sin((theta-delta)*6/10)+1)+10)
        cz=int(120*(math.sin((theta-delta)/5-PI)+1)+10)
