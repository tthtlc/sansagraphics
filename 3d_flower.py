...
def DrawEllipsoid(uistacks, uislices, fA, fB, fC):
	...
	#	s = -2*math.pi/2
		...
		for j in range(uistacks+1):
			s = sstep*j
			glVertex3f(fA * math.cos(varu*t)*math.cos(vart*t) * math.cos(myvars*s), fB * math.cos(vart*t) * math.sin(varu*t)*math.sin(myvars*s), fC * math.sin(vart*t)*math.sin(varu*t))
			glVertex3f(fA * math.cos(varu*t)*math.cos(vart*t+tstep) * math.cos(myvars*s), fB * math.cos(varu*t)*math.cos(vart*t+tstep) * math.sin(myvars*s), fC * math.sin(vart*t+tstep)*math.sin(varu*t))
		glEnd()
...
def display():
	...
	DrawEllipsoid(20, 60, 1.1, 2.1, 5.1)
