...
def DrawEllipsoid(uistacks, uislices, fA, fB, fC):
	...
	for i in range(uislices):
		...
		for j in range(uistacks+1):
			s = sstep*j
			glVertex3f(fA * math.cos(2*t) * math.cos(s), fB * math.cos(2*t) * math.sin(s), fC * math.sin(2*t))
			glVertex3f(fA * math.cos(2*t+tstep) * math.cos(s), fB * math.cos(2*t+tstep) * math.sin(s), fC * math.sin(2*t+tstep))
		glEnd()
