...
def DrawEllipsoid(uistacks, uislices, fA, fB, fC):
	...
	#	s = -2*math.pi/2
		...
		for j in range(2*uistacks+1):
			...
			glVertex3f(fA * math.sin(t*3)* math.cos(vart*t) * math.cos(myvars*s), fB * math.sin(t/2)* math.cos(vart*t) * math.sin(myvars*s), fC * math.sin(t/2)* math.sin(vart*t))
			glVertex3f(fA * math.sin(t*3+tstep)* math.cos(vart*t+tstep) * math.cos(myvars*s), fB *math.sin(t/2+tstep)* math.cos(vart*t+tstep) * math.sin(myvars*s), fC * math.sin(t/2+tstep)* math.sin(vart*t+tstep))
			if (j>uistacks):
