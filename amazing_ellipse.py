...
def DrawEllipsoid(uistacks, uislices, fA, fB, fC):
	...
	for i in range(uislices):
		...
		for j in range(uistacks+1):
			s = sstep*j
			glVertex3f(fA * math.cos(2*t) * math.cos(2*s)*math.sin(2*t), fB * math.cos(2*t) * math.sin(2*s), fC * math.sin(2*t))
			glVertex3f(fA * math.cos(2*t+tstep) * math.cos(2*s), fB * math.cos(2*t+tstep) * math.sin(2*s), fC * math.sin(2*t+tstep))
		glEnd()
...
def display():
	...
	DrawEllipsoid(90, 90, 1.1, 2.1, 5.1)
