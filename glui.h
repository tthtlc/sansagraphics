...
#define GLUI_HSLIDER_HEIGHT             16
#define GLUI_HSLIDER_WIDTH              130
...
public:
  ...
  void         adjust_glut_xy( int &x, int &y ) { y = h-y; x = x; };
...
public:
  ...
  virtual int mouse_down_handler( int local_x, int local_y ) 
    { local_x = local_y = local_x + local_y; return false; };
  virtual int mouse_up_handler( int local_x, int local_y, int inside )
    { local_x = local_y = inside = local_x+local_y+inside; return false; };
  ...
  virtual int key_handler( unsigned char key, int modifiers )
    { key = key; modifiers = modifiers; return false; };
  virtual int special_handler( int key,int modifiers )
    { key = modifiers = key+modifiers; return false; };

  virtual void update_size( void )     { };
  virtual void idle( void )            { };
  virtual int  mouse_over( int state, int x, int y )
		{ state = x = y = state+x+y; return false; };
  
  virtual void enable( void ); 
  virtual void disable( void );
  virtual void activate( int how )     { how = how; active = true; };
  ...
  virtual void set_text( char *t )   { t = t; };
...
public:
  ...
  GLUI_HSlider  *add_slider(int resizable=false,
			      int data_type=GLUI_HSLIDER_INT,
			      void *live_var=NULL,
			      int id=-1, GLUI_Update_CB callback=NULL );
  GLUI_HSlider  *add_slider_to_panel( GLUI_Panel *panel, 
				       int resizable=false,
				       int data_type=GLUI_HSLIDER_INT,
				       void *live_var=NULL,
				       int id=-1,
...
#define  GLUI_HSLIDER_ARROW_WIDTH  16
#define  GLUI_HSLIDER_ARROW_HEIGHT 16

#define  GLUI_HSLIDER_STATE_NONE   0
#define  GLUI_HSLIDER_STATE_UP     1
#define  GLUI_HSLIDER_STATE_DOWN   2
#define  GLUI_HSLIDER_STATE_BUTTON 3
#define  GLUI_HSLIDER_STATE_TROUGH_RIGHT 4
#define  GLUI_HSLIDER_STATE_TROUGH_LEFT 5
#define  GLUI_HSLIDER_STATE_BUTTON_LEFT 6
#define  GLUI_HSLIDER_STATE_BUTTON_RIGHT 7

#define  GLUI_HSLIDER_DEFAULT_GROWTH_EXP   1.05f


/************************************************************/
/*                                                          */
/*               HSlider class (container)                  */
/*                                                          */
/************************************************************/
 
class GLUI_HSlider : public GLUI_Control
{
public:
  int           state;
	int           button_is_resizable;
	float           button_left_edge;
	float           button_right_edge;
  int           bounce_target;
  int           new_button_center;
  int           old_button_center;
  int           data_type;
  int           callback_count;
  int           last_int_val;
  float         last_float_val;
  ...
  int  locate_click( int local_x, int local_y );
  void do_callbacks( void );
  void reconcile_edge( void );
  void do_click( void );

  void set_slider_val( float lval );
  void set_slider_val( float lval, float hval );
  void set_slider_val( int lval, int hval );
	float get_slider_val();
	int sync_slider();
	float find_val( float pct );

	private:
		int redge_to_pixel(float val);
		int ledge_to_pixel(float val);
		void move_button_pixels(int nval, int &oval);
		void move_val(float dx);
		float pixel_to_redge(int val);
		float pixel_to_ledge(int val);
		void update_slider_val();

	public:
  GLUI_HSlider( void ) {
    sprintf( name, "HSlider: %p", this );
    type         = GLUI_CONTROL_HSLIDER;
    h            = GLUI_HSLIDER_HEIGHT;
    w            = GLUI_HSLIDER_WIDTH;
	  button_is_resizable = false;
	  button_left_edge    = GLUI_HSLIDER_ARROW_WIDTH;
	  button_right_edge   = GLUI_HSLIDER_ARROW_WIDTH + 10;
		int_low			 = 0;
		int_high		 = w - 2*GLUI_HSLIDER_ARROW_WIDTH - 4;
