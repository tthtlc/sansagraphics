...
def DrawEllipsoid(uistacks, uislices, fA, fB, fC):
	...
	#	s = -2*math.pi/2
		...
		for j in range(uistacks+1):
			s = sstep*j
			myangle += 2*math.pi/28
			glVertex3f((fA * math.cos(vart*t) * math.cos(myvars*s))*math.sin(myangle), (fB * math.cos(vart*t) * math.sin(myvars*s))*math.sin(myangle), (fC * math.sin(vart*t))*math.cos(myangle)*math.sin(myangle))
			glVertex3f((fA * math.cos(vart*t+tstep) * math.cos(myvars*s))*math.sin(myangle), (fB *math.cos(vart*t+tstep) * math.sin(myvars*s))*math.sin(myangle), (fC * math.sin(vart*t+tstep))*math.sin(myangle)*math.cos(myangle))
		glEnd()
