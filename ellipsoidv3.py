...
def DrawEllipsoid(uistacks, uislices, fA, fB, fC):
	...
	for i in range(uislices):
		...
		for j in range(uistacks+1):
			s = sstep*j
			glVertex3f(fA * math.cos(3*t) * math.cos(2*s), fB * math.cos(3*t) * math.sin(s), fC * math.sin(3*t))
			glVertex3f(fA * math.cos(3*t+tstep) * math.cos(3*s), fB * math.cos(3*t+tstep) * math.sin(s), fC * math.sin(3*t+tstep))
		glEnd()
