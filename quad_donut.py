...
def DrawEllipsoid(uistacks, uislices, fA, fB, fC):
	...
	#	s = -2*math.pi/2
		for j in range(2*uistacks+1):
			s = sstep*j
			glVertex3f(fA * math.sin(t*4)* math.cos(t) * math.cos(s), fB * math.sin(t/2)* math.cos(t) * math.sin(s), fC * math.sin(t/2)* math.sin(t))
			glVertex3f(fA * math.sin(t*4+tstep)* math.cos(t+tstep) * math.cos(s), fB *math.sin(t/2+tstep)* math.cos(t+tstep) * math.sin(s), fC * math.sin(t/2+tstep)* math.sin(t+tstep))
			if (j>uistacks):
