...
class GrayCode(object):
    def __init__(self, nbits):
        ...
        self._graycode_int = []
        self.__generate()
        self.__convert_str2int()

    def __getitem__(self, i):
        #return self._grayCode[i]
        return self._graycode_int[i]

    def __str__(self):
        #return str(self._grayCode)
        return str(self._graycode_int)

    __repr__ = __str__

    def __iter__(self):            
        #return self._grayCode.__iter__()
        return self._graycode_int.__iter__()
    ...
    def __convert_str2int(self):
        self._graycode_int = []

        for i in range(len(self._grayCode)):
		#print int(self._grayCode[i],2)
        	self._graycode_int.append(int(self._grayCode[i],2))
        #print self._graycode_int
